var thisScript = $('script[src*=main]');
var UNIT = thisScript.attr('unit');

// 引数のハッシュ値(SHA-256)を取得する関数
...
$(function(){
    ...
    $.getJSON('/eticket/' + UNIT + '/application.json', function(json){
    ...
    // VOCABULARUY用の関数
    function funcVocabulary(json, app){
        ...
        var updateResult = function(score){
            ...
            if( countdown > (MAX_TIME / 1000) ){
                score = "Sorry Time Out!<br>Try Again!<br>" + score;
                soundRotation(app.sounds.timeoutSound);
            }
            // プラチナ
            else if( score >= 100 ){
                medalImagePath = json.materialPath + app.medalImage.platinum;
                soundRotation(app.sounds.platinumSound);
            }
            // 金
            else if( score >= 90 ){
                medalImagePath = json.materialPath + app.medalImage.gold;
                soundRotation(app.sounds.goldSound);
            }
            // 銀
            else if(score >= 80){
                medalImagePath = json.materialPath + app.medalImage.silver;
                soundRotation(app.sounds.silverSound);
            }
            // 銅
            else if(score >= 70){
                medalImagePath = json.materialPath + app.medalImage.bronze;
                soundRotation(app.sounds.bronzeSound);
            }
            // Try Again!
            else{
                score = "Try again!<br>" + score;
            }

            // スコアを表示
            if (medalImagePath) {
                $('#medalOverlay').html('<img src="' + medalImagePath + '" width="100" height="100"><br>' + score + ' 点');
            } else {
                $('#medalOverlay').html(score + ' 点'); // Sorry Time Out!が入ってる
            }

            $('#medalOverlay').css('display', 'inline');
        }
        // タイマー
        var voStep = function(){
    ...
    function funcToeicPart2(json, app){
        console.dir(app);
        var questionArr = ["question3Sound", "question4Sound", "question5Sound", "question6Sound"];
        // 音声のセット
    ...
    function funcToeicPart4(json, app){
                        ...
                        if(second <= p4time.gold){
                            ...
                            face_interval = setInterval( function(){
	                            ...
	                            if( $('#mainContent .p4-face.gold').css("background-image").indexOf("-off") < 0 ) {
		                            $('#mainContent .p4-face.gold').css("background-image", "url(" + json.materialPath + "image/gold-off.png)");
		                        } else {
			                        $('#mainContent .p4-face.gold').css("background-image", "url(" + json.materialPath + "image/gold-on.png)");
			                    }
                        ...
                        // silver
                            ...
                            face_interval = setInterval( function(){
	                            ...
	                            if( $('#mainContent .p4-face.silver').css("background-image").indexOf("-off") < 0 ) {
		                            $('#mainContent .p4-face.silver').css("background-image", "url(" + json.materialPath + "image/silver-off.png)");
		                        } else {
			                        $('#mainContent .p4-face.silver').css("background-image", "url(" + json.materialPath + "image/silver-on.png)");
			                    }
        ...
        // 問題の設定
        $('.p4-question_text').eq(0).text('9. ' + app.questionText[0]).data('sound', app.sounds.questionSound[0]);
        $('.p4-question_text').eq(1).text('10. ' + app.questionText[1]).data('sound', app.sounds.questionSound[1]);
    }
